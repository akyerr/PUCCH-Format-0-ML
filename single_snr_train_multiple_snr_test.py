...
filename_train = f"{path_to_data}pucch_data_slot_13_14_fading_{train_SNR}dB_{int(dataset_size/1000)}k.mat"
...
# save weights
weights_save_path = os.path.join(save_folder, 'weights_128.h5')
model.save_weights(weights_save_path)

plt.rcParams['axes.edgecolor'] = 'black'
plt.figure()
plt.plot(history.history['loss'], 's-r', label='Training', markevery=[0, 50, 100, 150, 199])
plt.plot(history.history['val_loss'], '^-b', label='Validation', markevery=[0, 50, 100, 150, 199])
...
plt.figure()
plt.plot(acc_perc, 's-r', label='Training', markevery=[0, 50, 100, 150, 199])
plt.plot(val_acc_perc, '^-b', label='Validation', markevery=[0, 50, 100, 150, 199])
