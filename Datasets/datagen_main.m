...
pucch.NSizeBWP = 273;
pucch.NStartBWP = 0;
pucch.SymbolAllocation = [5, 1]; % Time allocation
pucch.PRBSet = 0;
...
for i = 1: dataset_size
    ...
    if normalize == 1
        ...
        pucch_rb_pow = sum(abs(pucchRx).^2)/12;
        pucchRx = pucchRx/sqrt(pucch_rb_pow);
    end
...
if strcmpi(tx_type, 'true')
    save(['./pucch_data_slot_', num2str(SLOT), '_symb_', num2str(SYMB),'_', channel_model, '_', num2str(SNR), 'dB_', num2str(dataset_size/1000),'k.mat'], 'X', 'Y');
else
    save(['./noise_data_slot_', num2str(SLOT), '_symb_',num2str(SYMB), '_', num2str(SNR), 'dB_', num2str(dataset_size/1000),'k.mat'], 'X', 'Y');
end
