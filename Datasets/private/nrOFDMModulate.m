...
function [grid,info,nSlot,hasSampleRate] = validateInputs(varargin)
    ...
    if (isCarrierSyntax) % nrOFDMModulate(CARRIER,GRID,..)
        ...
        opts = coder.const(nr5g.internal.parseOptions([fcnName '(carrier,...'],optNames,varargin{3:end}));
    ...
    if (~isempty(coder.target) && isCarrierSyntax && hasSampleRate)
        coder.internal.errorIf(true,'nr5g:nrOFDMModulate:CompilationCarrierSampleRate',sprintf('%g',opts.SampleRate),'IfNotConst','Fail');
    end
