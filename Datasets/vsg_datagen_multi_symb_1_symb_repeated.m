...
SLOT = 14; % Each frame has 20 slots
carrier.NSlot = SLOT;
format = 0;
...
Y = zeros(dataset_size, num_symb_repeat);
...
for i = 1: dataset_size
    ...
    for s = 1: num_symb_repeat
        ...
        pucchGrid(1:12, s) = pucchSymbols;
        Y(i, s) = applied_CS;
    end
    % place the UCI data in the grid
%     pucchGrid(pucchIndices) = pucchSymbols;

%     pucchGrid(1:12, 1:13) = repmat(pucchSymbols, 1, 13);

    % OFDM Modulation
    txWaveform = OFDMModulate(carrier, pucchGrid);
    save(['./Files_for_vsg_multi_symb_1_repeated_13times_slot_', num2str(SLOT), '/pucch_f0_txwaveform_', num2str(i)], 'txWaveform');
%     Y(i, :) = applied_CS;
end

save(['./Files_for_vsg_multi_symb_1_repeated_13times_slot_', num2str(SLOT),'/pucch_f0_applied_CS'], 'Y');
