function info = nrPUCCHHoppingInfo(cp,nslot,nid,groupHopping,initialCS,seqCS)
...
%             corresponds to each hop in a slot.
%   Alpha   - Vector of cyclic shifts of all symbols in a given slot.
...
%#codegen
    ...
    nRBSC = 12;
    alpha = 2*pi*(mod(double(initialCS)+double(seqCS)+ncs,nRBSC))./nRBSC;

    alpha_ML = mod(double(initialCS)+double(seqCS)+ncs,nRBSC);

    % Combine the information into a structure
    info.U        = u;
    info.V        = v;
    info.Alpha    = alpha;
    info.alpha_ML = alpha_ML;
    info.FGH      = fgh;
    info.FSS      = fss;
