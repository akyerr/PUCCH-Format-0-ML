function [uciBits,errVec] = nrUCIDecode(uciLLRs, A, folder, format, config_no, varargin)
...
%   encUCI = nrUCIEncode(uci,E,modScheme);
...
%     narginchk(2,5);
    ...
    if A>=12 % Polar decoding (both Parity-Check and CRC-Aided)
        ...
        for cIdx = 1:C
            ...
            testvectors_text([folder, 'cb_', num2str(cIdx), '_llrs_f', num2str(format), '_', num2str(config_no), '.txt'], uciCW, 'same', true);
            save([folder, 'cb_', num2str(cIdx), '_llrs_f', num2str(format), '_', num2str(config_no), '.mat'], 'uciCW')

            % Rate recovery, Sections 6.3.1.4.1/6.3.2.4.1, 5.4.1 [1]
            recBlk = nrRateRecoverPolar(uciCW,K,N,iBIL);
            
            testvectors_text([folder, 'cb_', num2str(cIdx), '_rate_rec_out_f', num2str(format), '_', num2str(config_no), '.txt'], recBlk, 'same', true);
            save([folder, 'cb_', num2str(cIdx), '_rate_rec_out_f', num2str(format), '_', num2str(config_no), '.mat'], 'recBlk')

            % Polar decoding, Sections 6.3.1.3.1/6.3.2.3.1, 5.3.1 [1]
            decBlk(:,cIdx) = nrPolarDecode(recBlk,K,Er,L,nMax,iIL,Lcrc);
            polar_dec_out = decBlk(:,cIdx);
            
            testvectors_text([folder, 'cb_', num2str(cIdx), '_polar_dec_out_f', num2str(format), '_', num2str(config_no), '.txt'], polar_dec_out, 'same', true);
            save([folder, 'cb_', num2str(cIdx), '_polar_dec_out_f', num2str(format), '_', num2str(config_no), '.mat'], 'polar_dec_out')
        end
        ...
        if A<3  % for 1,2
            uciBits = smallDecode12(uciLLRs,A,modulation, folder, format, config_no);
        else    % for 3...11
            uciBits = smallDecode311(uciLLRs,A, folder, format, config_no);
        end
...
% Check inputs
    ...
    % Validate input length which must be greater than minE (based on A)
...
function decBits = smallDecode12(uciLLRs,A,modScheme, folder, format, config_no)
%smallDecode12 Small block length decoding for 1 or 2 bits

    testvectors_text([folder, 'small_llrs_f', num2str(format), '_', num2str(config_no), '.txt'], uciLLRs, 'same', true);
    save([folder, 'small_llrs_f', num2str(format), '_', num2str(config_no), '.mat'], 'uciLLRs')
    ...
    testvectors_text([folder, 'small_rate_rec_out_f', num2str(format), '_', num2str(config_no), '.txt'], rec, 'same', true);
    save([folder, 'small_rate_rec_out_f', num2str(format), '_', num2str(config_no), '.mat'], 'rec')
    % Decode: ML or exhaustive search
    %   Form all soft messages
    m = 2^A;
    allMsgs = de2bi(0:m-1,A); % one per row
    ...
    testvectors_text([folder, 'small_dec_out_f', num2str(format), '_', num2str(config_no), '.txt'], decBits, 'same', true);
    save([folder, 'small_dec_out_f', num2str(format), '_', num2str(config_no), '.mat'], 'decBits')
end

function decBits = smallDecode311(uciLLRs,A, folder, format, config_no)
%smallDecode311 Small block length decoding for 3...11 bits
    ...
    testvectors_text([folder, 'small_all_msgs_enc_f', num2str(format), '_', num2str(config_no), '.txt'], allEncMsgs, 'same', true);
    save([folder, 'small_all_msgs_enc_f', num2str(format), '_', num2str(config_no), '.mat'], 'allEncMsgs')
